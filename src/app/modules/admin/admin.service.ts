...
export const toggleAgentStatusSvc = async (id: string) => {
  ...
  // cycle: pending -> approved -> suspended -> approved...
...
export const updateAdminProfileSvc = async (adminId: string, payload: Partial<{ name: string; phone: string; password: string }>) => {
  const admin = await User.findById(adminId);
  if (!admin) throw new Error("Admin not found");
  if (payload.name) admin.name = payload.name;
  if (payload.phone) admin.phone = payload.phone;
  if (payload.password) {
    // TODO: hash password as per your auth system
